...
const EditUserModals = ({
	isOpen,
	close,
	user: userDataFromProps
}: ModalBaseProps & { user: UserData }) => {
	...
	const handlePrefixChangeInternal = (
	...
	const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
		...
		if (!userData.phone) {
			...
			newErrors.phoneError =
				errorMessages[CustomErrorCodes.INVALID_PHONE_NUMBER]
			isValid = false
		}
		...
		try {
			const result =
				await updateProfileServices().updateUserInfo(updatedUserData)
			if (result.success) {
