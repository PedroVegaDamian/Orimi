    ...
    <section>
      ...
      {orders.length === 0 ? (
        <div className="text-center m-4">
          ...
          <p>
            Select a product, add it to your cart, and complete the purchase.
          </p>
        </div>
      ) : (
        orders.map(order => (
          <div
            key={order.id}
            className="w-[90%] mx-auto pt-[24px] pb-[24px] flex flex-col items-start md:items-start"
          >
            <div className="flex flex-col items-start md:items-start mb-4 md:mb-0 md:w-full">
              ...
              <p>
                Price:{' '}
                <span className="text-primary_800_color font-bold">
                  $
                  {order.cart
