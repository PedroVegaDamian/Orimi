...
const AddressListPage = () => {
	...
	const isModalOpen =
		editAddressModalOpen || deleteAddressModalOpen || newAddressModalOpen
	useBodyScrollLock(isModalOpen)

	useEffect(() => {
		if (user && user.id) {
			fetchAddressesFromFirebase(user.id)
				.then(addresses => {
		...
		async (addressId: string, address: Partial<Address>) => {
			...
			if (completeAddress.isDefault) {
				const promises = updatedAddresses.map(addr =>
					addr.id !== addressId
						? updateAddress(user.id, addr.id, { ...addr, isDefault: false })
						: Promise.resolve()
				)
		...
		try {
			...
			if (selectedAddress) {
				for (const addr of updatedAddresses) {
					await updateAddress(user.id, addr.id, {
						...addr,
					...
					{isFetchingAddresses ? (
						...
						addresses.map(address => (
							<div
								...
								className={
									address.isDefault
										? 'border-primary_800_color border-2 rounded-10 p-4'
										: ''
								}
							>
								<div className="flex">
									<label
										htmlFor={`defaultAddress-${address.id}`}
									...
									>
										<Radio
											...
											className={
												address.isDefault
													? 'text-primary_800_color ring-primary_800_color focus:ring-primary_800_color'
													: ''
											}
										/>
										<p className="ml-2">
